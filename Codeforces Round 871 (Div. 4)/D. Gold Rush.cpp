...
//bool solve3(ll n, ll m) {
//
//    if (m == n || m > n || n % 3) return true;
//    else return false;
//}

ll m;
bool breakCheck ( ll n) {
    if(m==n || m>n || n%3) return  true;
    else return  false;
}
bool fac(ll n) {
    if(breakCheck(n) && m == n) return true;
    else if(breakCheck(n)) return false;
    return fac(1* (n / 3)) || fac(2* (n / 3)) ;
}
