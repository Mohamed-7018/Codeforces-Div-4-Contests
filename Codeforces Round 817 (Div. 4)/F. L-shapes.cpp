/* Depression i snot dp side effect of illness,
  it is dp side effect of dying  */
// author  :  _Depreesed
...
void dfs (ll i,ll j) {
    ...
    F(k,0,4) {
        ll x = i+dx[k];
        ll y = j+dy[k];
        if(x<0 or x>=n or y<0 or y>=m or adj[x][y] !='*' or vis[x][y])
            continue;
        else dfs(x,y);
    }
}
          ...
          if(adj[i][j] == '*' and !vis[i][j]) {
              ...
              for (auto it:tmp) {
                  F(k,0,8) {
                      ll nx = it.fi+dx[k];
                      ll ny = it.se + dy[k];
                      if(nx < 0 || ny < 0 || nx >= n || ny >= m || adj[nx][ny] != '*' || tmp.count({nx,ny}))continue;
                      else {
                          PNO; return;
                      }
                  }
